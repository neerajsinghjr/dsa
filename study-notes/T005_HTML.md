...
There are many different attributes that we can specify for HTML forms. 
...
#### Element : Fieldset
The `<fieldset>` element groups related form elements. Inside the `<fieldset>`, 
a `<legend>` element sets the caption at the top.
...
**NOTE: Image Tag with `alt` attribute** :- 

The `alt` attribute in an img tag is to provide a description of the image. 
This is particularly useful in situations when our image is not properly loaded 
on the screen. Also, the alt text is used by screen readers to explain the 
...
Here, `<h1>` at the beginning is called the opening tag, the 'Hello, World!' 
text is the content and the `</h1>` at the end is called the closing tag.
